...
struct StoryBrain {
  ...
  mutating func loadNextStory(choice: String) {
    if choice == stories[currentStory].choice1 {
      self.currentStory = stories[currentStory].choice1Destination
    } else {
      self.currentStory = stories[currentStory].choice2Destination
    }
  }
}
